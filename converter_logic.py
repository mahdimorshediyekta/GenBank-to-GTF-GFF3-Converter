...
if __name__ == "__main__":
    if len(sys.argv) < 3 or len(sys.argv) > 4:
        logging.error("Usage: python converter.py <input_genbank_file> <output_file> [format]")
        logging.error("  format: 'gtf' (default) or 'gff3'")
        sys.exit(1)

    input_gb_file = sys.argv[1]
    output_file = sys.argv[2]
    output_format = sys.argv[3] if len(sys.argv) == 4 else "gtf"

    try:
        convert_genbank_to_annotation_format(input_gb_file, output_file, output_format)
        logging.info(f"Successfully converted '{input_gb_file}' to '{output_file}' in {output_format.upper()} format.")
    except Exception as e:
